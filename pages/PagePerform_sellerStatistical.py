...
def Page():
    ...
    if request.method == "POST":
        relateObj["result"] = db.session.execute(GenerateAllSql()).fetchall()
    return relateObj
...
def GenerateAllSql():
    ...
    SELECT 
        ...
        SUM(totalGotMoney) AS totalGotMoney, 
        SUM(totalMoney) AS totalMoney,
        CASE 
            WHEN SUM(totalMoney) <> 0 THEN (SUM(totalGotMoney) * 100 / SUM(totalMoney))
            ELSE 0
        END AS gotPercent
    FROM
    (
    ...
    GROUP BY tt.sellerName, tt.sellerId
    HAVING SUM(totalMoney) > 0''' % GenerateSql()
...
def GenerateSql():
    ...
    ON co.id = i.id
    GROUP BY p.id, p.departTime, p.operatorId, p.leader, l.name, u.name, p.peopleNum, p.profitTypeId, u.id, su.id, p.sellerId
